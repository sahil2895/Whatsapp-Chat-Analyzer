...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        try:
            ...
            df[['Sentiment', 'Confidence']] = df['message'].apply(lambda msg: pd.Series(helper.analyzesenti(msg,df)))
            ...
            st.dataframe(df[['user', 'message', 'Sentiment', 'Confidence']])
            
        except Exception as e:
            st.error(f"Error in sentiment analysis: {e}")
            st.write("Could not perform sentiment analysis. Please check your data or try again.")
        
        st.write("## 1. Predict Next Message")
        user_input = st.text_area("Enter recent chat context:")
        if st.button("Predict Message"):
            if user_input:
                prediction = helper.predict_next_message(user_input)
                st.write("**Predicted Response:**", prediction)


        st.write("## 2. Generate Follow-Up Message")
        if st.button("Generate Follow-Up"):
            followup = helper.generate_followup(df['messages'])
            st.write("**Generated Follow-Up:**", followup)

        st.write("## 5. Detect Unusual Activity")
        if st.button("Detect Unusual Activity"):
            unusual_activity = helper.detect_unusual_activity(df['messages'])
            st.write("**Detected Anomalies:**", unusual_activity)
